  ...
  const togglePart = React.useMemo(() => {
    if (isLogin) {
        ...
        {({ errors, isSubmitting }) => (
          <Form className="flex flex-1 flex-col  justify-center space-y-5 max-w-md">
            <div className="flex flex-col space-y-2 text-center">
              ...
              <p className="text-md md:text-xl">Welcome from Kipling English center, Myanmar!</p>
            </div>
      ...
      return <Formik
        ...
        {({ errors, isSubmitting }) => (<Form className="flex flex-1 flex-col  justify-center space-y-5 max-w-md">
          <div className="flex flex-col space-y-2 text-center">
            <h2 className="text-3xl md:text-4xl font-bold">Sign Up an account</h2>
            <p className="text-md md:text-xl">Please fill out the form and click Sign Up button!</p>
          </div>
          <div className="flex flex-col max-w-md space-y-5">
            ...
            <div className="form-control">
              <Field type="password" placeholder="Confirm Password" name="confirm"
                className={` ${errors.confirm ? 'input-error border-red' : 'border-black'} flex px-3 py-2 md:px-4 md:py-3 border-2 rounded-lg font-medium placeholder:font-normal`} />
              <p className={`text-sm text-red-600 dark:text-red-500 ${errors.confirm ? 'block' : 'hidden'}`}>{errors.confirm}</p>
            </div>
            <div className="form-control">
              <Field type="text" placeholder="Phone number" name="phoneNumber"
                className={` ${errors.phoneNumber ? 'input-error border-red' : 'border-black'} flex px-3 py-2 md:px-4 md:py-3 border-2 rounded-lg font-medium placeholder:font-normal`} />
              <p className={`text-sm text-red-600 dark:text-red-500 ${errors.phoneNumber ? 'block' : 'hidden'}`}>{errors.phoneNumber}</p>
            </div>
            <button className="flex items-center justify-center flex-none px-3 py-2 md:px-4 md:py-3 border-2 rounded-lg font-medium border-black bg-black text-white">
              <ClipLoader
                ...
                data-testid="loader"
              />Sign up</button>
            {<p className="text-lg text-center lg:hidden block">Already a member?<span onClick={(event) => setIsLogin(true)} className='pl-2 text-sky-600 cursor-pointer'>Sign in <FontAwesomeIcon icon={faUpRightFromSquare}></FontAwesomeIcon></span></p>}
          </div>
        </Form>)}
      </Formik>
    }
